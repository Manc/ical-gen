import {addOrGetCustomAttributes, checkEnum, checkNameAndMail, escapeICalValue} from './utils';
...
export default class ICalAttendee {
	...
	toString (): string {
		...
		// CN / Name
		if (this.data.name) {
			g += ';CN="' + escapeICalValue(this.data.name) + '"';
		}

		// EMAIL
		if (this.data.email && this.data.mailto) {
			g += ';EMAIL=' + escapeICalValue(this.data.email);
		}
		...
		g += ':MAILTO:' + escapeICalValue(this.data.mailto || this.data.email) + '\r\n';
